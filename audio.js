...
function createRecognizer(audiofilename, audioLanguage) {
	...
	speechConfig.setProperty(sdk.PropertyId.SpeechServiceConnection_InitialSilenceTimeoutMs, "600000");
	speechConfig.setProperty(sdk.PropertyId.Conversation_Initial_Silence_Timeout, "600000");
	speechConfig.setProperty(sdk.PropertyId.SpeechServiceConnection_EndSilenceTimeoutMs, "600000");
	...
	outputStream.once("open", () => {
		...
		recognizer.recognized = (s, e) => {
			if (e.result.reason === sdk.ResultReason.NoMatch) {
				...
				outputStream.write(
					`${parseTime(e.result.offset)} --> ${parseTime(e.result.offset + e.result.duration)}\r\n`
				);
